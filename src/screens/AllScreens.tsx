      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        <View style={[styles.card,{borderColor:COLORS.success,borderWidth:1}]}>
          <View style={{flexDirection:'row',alignItems:'center',gap:12}}>
            ...
            <View><Text style={styles.cardTitle}>Estado actual</Text><Text style={{color:COLORS.success,fontWeight:'700'}}>ENTORNO SEGURO</Text></View>
          </View>
        </View>
      </ScrollView>
    </SafeAreaView>
...
export function PaymentScreen() {
  const plans = [{name:'BASICO',price:'$4.99',color:COLORS.secondary},{name:'PRO',price:'$12.99',color:COLORS.cyan},{name:'FAMILIA',price:'$24.99',color:'#8B5CF6'}];
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll}>
        <Text style={styles.pageTitle}>Elige tu plan</Text>
        {plans.map((p,i)=>(
          <View key={i} style={[styles.planCard,{borderColor:p.color}]}>
            <Text style={[styles.planName,{color:p.color}]}>{p.name}</Text>
            <Text style={styles.planPrice}>{p.price}<Text style={styles.planPeriod}>/mes</Text></Text>
            <TouchableOpacity style={[styles.planBtn,{backgroundColor:p.color}]} onPress={()=>Alert.alert('Plan',p.name)}>
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll}>
        <View style={styles.profileHeader}>
          <View style={styles.avatar}><MaterialCommunityIcons name="account" size={52} color={COLORS.cyan} /></View>
