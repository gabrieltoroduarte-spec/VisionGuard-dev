...
export function HomeScreen() {
  ...
  const startScan = () => {
    setScanning(true); setStatus('Analizando entorno...');
    Vibration.vibrate(200);
    setTimeout(()=>{ setScanning(false); setStatus('Entorno seguro'); Alert.alert('Listo','No se detectaron obstaculos'); },3000);
  };
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <TouchableOpacity style={styles.sos} onPress={()=>{Vibration.vibrate([0,500,200,500]);Alert.alert('SOS','Contactando emergencias...');}}>
          <MaterialCommunityIcons name="phone-alert" size={28} color="#FFF" />
          <Text style={styles.sosText}>BOTON DE EMERGENCIA SOS</Text>
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  sos:{backgroundColor:'#DC2626',borderRadius:16,padding:20,flexDirection:'row',justifyContent:'center',alignItems:'center',gap:12},
